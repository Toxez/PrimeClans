...
public abstract class SqlDb implements Db {
    ...
    public CompletableFuture<Map<String, Clan>> loadAll() {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Connection c = ds.getConnection()) {
                ...
                Map<UUID, Set<String>> memberPerms = new HashMap<>();
                try (PreparedStatement ps = c.prepareStatement("SELECT * FROM member_perms")) {
                    ...
                    while (rs.next()) {
                        ...
                        if (perm != null && !perm.isBlank()) {
                            memberPerms.computeIfAbsent(uuid, k -> new HashSet<>())
                                    .add(perm.toUpperCase());
                        }
                    }
                }
    ...
    @Override
    public void removeMemberPerm(UUID uuid, String permKey) {
        Task.async(() -> {
            try (Connection c = ds.getConnection();
