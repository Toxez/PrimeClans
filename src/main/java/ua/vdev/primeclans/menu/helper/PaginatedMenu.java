...
public abstract class PaginatedMenu implements Menu {
    ...
    protected final Map<Integer, List<MenuAction>> leftActions =
        new HashMap<>();
    protected final Map<Integer, List<MenuAction>> rightActions =
        new HashMap<>();
    ...
    private void loadPaginationButtons(
        ...
        if (currentPage > 0) {
            buildPaginationItem(
                ...
                List<MenuAction> action = List.of(p ->
                    MenuManager.openMenu(p, prevPage())
                );
        ...
        if (currentPage < totalPages - 1) {
            buildPaginationItem(
                ...
                List<MenuAction> action = List.of(p ->
                    MenuManager.openMenu(p, nextPage())
                );
    ...
    private Optional<org.bukkit.inventory.ItemStack> buildPaginationItem(
        ConfigurationSection menuSection,
        String path,
        Map<String, String> placeholders
    ) {
    ...
    protected abstract void loadStaticItems(
        Inventory inventory,
        ConfigurationSection menuSection,
        Map<String, String> placeholders
    );

    protected abstract void loadPageItems(
        Inventory inventory,
        ConfigurationSection menuSection,
    ...
    protected abstract Map<String, String> buildPlaceholders(
        int currentPage,
        int totalPages
    );
